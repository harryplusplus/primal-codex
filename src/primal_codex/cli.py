...
Reads settings from `{_PRIMAL_CODEX_CONFIG}` (default `{_PRIMAL_CODEX_CONFIG_DEFAULT}`) and partially updates `{_CODEX_CONFIG}` (default `{_CODEX_CONFIG_DEFAULT}`). A backup of any existing target file is saved as `<path>.bak`.

### Environment Variables
- **`{_PRIMAL_CODEX_HOME_ENV_KEY}`** *(optional)* — Path to the Primal Codex home directory. Defaults to `{_PRIMAL_CODEX_HOME_DEFAULT}` if unset.
- **`{_CODEX_HOME_ENV_KEY}`** *(optional)* — Path to the Codex home directory. Defaults to `{_CODEX_HOME_DEFAULT}` if unset."""  # noqa: E501
)
def codex() -> None:
